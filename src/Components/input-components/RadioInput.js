...
const RadioInput = (props) => {
  ...
  const inputBoxClasses = clsx('input-box', 'h-[45px] flex items-center cursor-pointer mt-[.5em]  hover:border-lime', {
  ...
  return (
    <div className={inputBoxClasses} ref={divRef} onClick={handleClick}
    > 
      <div className='relative flex items-center'>
        <input 
        type='radio' 
        className={`appearance-none bg-white w-[1.2em] h-[1.2em] border 
          border-[#4e4e4e] rounded-[50%] mx-[1.2em] cursor-pointer checked:border-lime`}
        name='mortgageType'
